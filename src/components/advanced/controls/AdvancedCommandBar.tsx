// ============================================================================
// AdvancedCommandBar — Floating AI toolbar with quick access to ALL panels
...
export default function AdvancedCommandBar() {
  const {
    ...
    <>
      {tools.map((tool) => (
        <button
          key={tool.label}
          onClick={tool.action}
          className={`advanced-toolbar-btn ${tool.color}`}
          title={tool.label}
        >
  ...
  return (
    <motion.div
      ...
      className="advanced-command-bar"
    >
      <div className="flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-1.5 pr-2 border-r border-slate-200 mr-0.5">
          ...
          <span className="text-[10px] font-bold text-slate-600 hidden lg:inline">ADVANCED AI</span>
        </div>
        ...
        <button
          ...
          className={`advanced-toolbar-btn ${particlesEnabled ? 'text-cyan-600 bg-cyan-50' : ''}`}
        ...
        <button
          ...
          className={`advanced-toolbar-btn ${commandCenterMode ? 'text-emerald-600 bg-emerald-50' : ''}`}
