...
const MOCK_AUDIT: AuditEntry[] = [
  ...
  { id: 'a4', timestamp: '2026-04-03T20:20:00Z', user: 'Emily Rodriguez', action: 'Permission Change', category: 'access', details: 'Role changed from "viewer" to "editor" for user m.park@nexus.ai', severity: 'high', ip: '192.168.1.87' },
...
export default function AuditLogPanel() {
  const { auditPanelOpen, toggleAuditPanel } = useNexusStore();
        ...
        >
          <div className="p-6">
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">System activity trail</p>
                </div>
              </div>
              <div className="flex gap-2">
                <button className="nexus-icon-btn" title="Export Logs">
                  <Download className="w-4 h-4" />
                </button>
                <button onClick={toggleAuditPanel} className="nexus-close-btn">
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-4 gap-2 mb-4">
              {[
                { label: 'Total', value: MOCK_AUDIT.length, color: 'text-white' },
                { label: 'Auth', value: MOCK_AUDIT.filter(a => a.category === 'auth').length, color: 'text-blue-400' },
                { label: 'Data', value: MOCK_AUDIT.filter(a => a.category === 'data').length, color: 'text-cyan-400' },
                { label: 'High Risk', value: MOCK_AUDIT.filter(a => a.severity === 'high').length, color: 'text-rose-400' },
              ].map((stat) => (
                <div key={stat.label} className="nexus-metric-card text-center py-2">
            ...
            <div className="flex gap-2 mb-4">
              <div className="flex-1 flex items-center gap-2 bg-white/5 rounded-xl px-3 py-2 border border-white/10 focus-within:border-indigo-500/40 transition-colors">
              ...
              <select
                ...
                className="bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs text-slate-300 outline-none cursor-pointer [&>option]:bg-slate-900"
              >
                ...
                {filteredLogs.map((entry, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="relative flex gap-3 p-3 rounded-xl hover:bg-white/3 transition-colors group"
                    >
                      {/* Timeline dot */}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-slate-400 leading-relaxed mb-1">{entry.details}</p>
