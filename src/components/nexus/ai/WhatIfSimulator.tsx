...
export default function WhatIfSimulator({ numericColumns = [], datasetSize = 100 }: Props) {
  const { whatIfPanelOpen, toggleWhatIfPanel } = useNexusStore();
    ...
    <AnimatePresence>
      {whatIfPanelOpen && (
        <motion.div initial={{ opacity: 0, x: 400 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 400 }}
          ...
          className="fixed right-0 top-0 bottom-0 w-[420px] z-[60] nexus-panel-glass overflow-y-auto">
          <div className="p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-2">
                <button onClick={() => setParams(defaultParams)} className="nexus-icon-btn"><RotateCcw className="w-4 h-4" /></button>
                <button onClick={toggleWhatIfPanel} className="nexus-close-btn"><X className="w-4 h-4" /></button>
              </div>
            </div>

            <div className="nexus-metric-card mb-4">
            ...
            <div className="space-y-4 mb-6">
              {params.map((param, i) => {
                ...
                return (
                  <motion.div key={param.name} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="nexus-metric-card">
                    ...
                    <input type="range" min={param.min} max={param.max} value={param.currentValue}
                      ...
                      className="nexus-range-slider w-full" />
