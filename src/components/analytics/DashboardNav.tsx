          ...
          <div className="w-full h-full max-w-[1800px] mx-auto flex items-center justify-between px-4 lg:px-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                ...
                >
                  <div className="w-6 h-6 rounded-full bg-gradient-to-br from-indigo-500 to-cyan-500 flex items-center justify-center text-[10px] font-bold text-white shadow-[0_0_10px_rgba(59,130,246,0.3)]">
                    {username.charAt(0).toUpperCase()}
                  </div>
                  <ChevronDown className="h-3 w-3 hidden sm:block" />
                </button>
                {profileOpen && (
                  <div className="absolute right-0 top-full mt-2 w-48 bg-slate-900/95 backdrop-blur-xl rounded-xl shadow-[0_0_30px_rgba(0,0,0,0.8)] border border-white/10 overflow-hidden z-50 animate-fade-up">
                    <div className="px-4 py-3 border-b border-white/5 bg-black/40">
                      <p className="text-xs font-bold text-white font-mono">{username}</p>
                      <p className="text-[10px] text-cyan-400 font-mono tracking-widest uppercase mt-0.5">Enterprise Admin</p>
                    </div>
                    <button onClick={handleLogout} className="w-full flex items-center gap-2.5 px-4 py-3 text-xs text-pink-400 hover:bg-pink-500/10 transition-colors font-mono font-bold tracking-wide group">
                      <LogOut className="h-3.5 w-3.5 group-hover:drop-shadow-[0_0_5px_currentColor]" />
                      Terminate Session
                    </button>
                  </div>
                )}
